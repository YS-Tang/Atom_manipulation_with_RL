...
import numpy as np
from .get_atom_coordinate_tys import get_atom_coordinate_pixel
import findiff
...
class RealExpEnv:
    ...
    def __init__(self,
        ...
        self.pixel2nm_factor = self.len_nm/self.pixel
    ...
    def scan_atom(self):
        ...
        drift_nm = self._calc_drift((img_forward, img_backward)) * self.pixel2nm_factor
        print(f'图像漂移: {drift_nm}纳米')
        ...
        # offset进行漂移校正，relative_nm相对于当前offset所在面的左上角(0,0)位置不变，absolute_nm经过offset校正
        atom_relative_nm_f = self.pixel2nm_factor * atom_pixel_forward
        atom_absolute_nm_f = atom_relative_nm_f + offsets
        
        atom_relative_nm_b = self.pixel2nm_factor * atom_pixel_backward
        atom_absolute_nm_b = atom_relative_nm_b + offsets
